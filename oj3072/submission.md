...
```text
นับสระที่อยู่ในข้อความ

input:
ข้อความ(string)

output:
จำนวนสระที่มี แยกว่าแต่ละตัวมีจำนวนเท่าไหร่

Constraints:
หากสระไหนไม่มี จะไม่แสดงผลสระนั้น จะแสดงผลเฉพาะสระที่มีเท่านั้น
```
...
```text
Step 1: รับค่าข้อความเป็น string และเปลี่ยนให้เป็นตัวพิมพ์เล็กทั้งหมดด้วย .lower()
Step 2: สร้างลิสต์ vowel = ["a" , "e" , "i" , "o" , "u"]
Step 3: กำหนดให้ตัวแปร x = 0
Step 4: กำหนดให้ตัวแปรสำหรับนับ (ได้แก่ a , e , i , o , u) เป็น 0 ทั้งหมด
Step 5: กำหนดให้ stop = len(string) ใช้สำหรับกำหนดจุดสิ้นสุดของขั้นตอนเช็คตัวอักษรแต่ละตัว
Step 6:  เช็คว่าตัวอักษรแต่ละตัวเป็นสระอะไร โดยใช้ for x in range(0,stop) โดยให้ x เพิ่มทีละ 1
Step 7: ถ้า x ยังอยู่ในช่วงที่กำหนด ให้ check = ตำแหน่งตัวอักษรที่กำหนด และเช็คต่อว่า
Step 8: check เป็น a หรือไม่ ถ้าใช่ ให้ a += 1
Step 9: check เป็น e หรือไม่ ถ้าใช่ ให้ e += 1
Step 10: check เป็น i หรือไม่ ถ้าใช่ ให้ i += 1
Step 11: check เป็น o หรือไม่ ถ้าใช่ ให้ o += 1
Step 12: check เป็น u หรือไม่ ถ้าใช่ ให้ u += 1
Step 13: ถ้า x ไม่อยู่ในช่วงที่กำหนด ให้เช็คต่อว่า
Step 14: a >= 1 หรือไม่
Step 15: ถ้าใช่ให้ print ค่า a
Step 16: e >= 1 หรือไม่
Step 17: ถ้าใช่ให้ print ค่า e
Step 18: i >= 1 หรือไม่
Step 19: ถ้าใช่ให้ print ค่า i
Step 20: o >= 1 หรือไม่
Step 21: ถ้าใช่ให้ print ค่า o
Step 22: u >= 1 หรือไม่
Step 23: ถ้าใช่ให้ print ค่า u
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก เพราะเข้าใจง่ายและไม่ซับซ้อนมาก โปรแกรมแสดงผลตามที่โจทย์ต้องการ
```
...
```text
เคสปกติ ที่ข้อความเป็นตัวพิมพ์เล็กทั้งหมด
```
...
```text
เคสที่ข้อความมีทั้งตัวพิมพ์ใหญ่และตัวพิมพ์เล็กผสมกัน
```
...
```text
เคสที่ข้อความทั้งหมดเป็นตัวพิมพ์ใหญ่
```
