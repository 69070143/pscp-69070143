...
```text
หาว่าจำนวนเต็มบวกตั้งแต่ N ถึง 0 มีค่าใดบ้างที่หารด้วย 10 ลงตัว

input:
รับค่า N

output:
แสดงจำนวนเต็มบวกตั้งแต่ N ถึง 0 ที่หารด้วย 10 ลงตัว (คั่นข้อมูลแต่ละตัวด้วย " ")
```
...
```text
Step 1: รับค่า N
Step 2: กำหนดให้ result = []
Step 3: เช็ค for i in range(0,N+1) เป็นจริงไหม โดยให้ i เพิ่มทีละ 10
Step 4: ถ้าจริง จะเช็คว่า i % 10 == 0 หรือไม่
Step 5: ถ้าหารลงตัว ให้เพิ่มค่า i เข้าในลิสต์ result
Step 6: ถ้า i ไม่อยู่ในช่วง ให้แสดงผลลิสต์ result

แต่ฉันไม่รู้ว่าต้องสั่ง print ยังไงให้ได้รูปแบบตามที่โจทย์ต้องการ ก่อนถามเพื่อน
```
...
```text
เหมือนวิธีของแผนแรก แต่ฉันแก้คำสั่ง print โดยใช้
print(*result[::-1])
เพราะโจทย์ให้เรียงคำตอบจากมากไปน้อยและเว้นด้วย " "
```
...
```text
เลขหาร 10 ลงตัว และจำนวนคำตอบเยอะ
```
...
```text
เลขหาร 10 ไม่ลงตัว และจำนวนคำตอบน้อย
```
...
```text
ช่วยสอนการ print ข้อมูลที่อยู่ในลิสต์โดยที่ไม่แสดงผล  "[]" และ "," พร้อมมีช่องว่างคั่นข้อมูลแต่ละตัว ด้วยการใช้ print(*list)
```

คุณยังทำอะไรด้วยตนเอง

```text
นำสิ่งที่เพื่อนสอนไปประยุกต์ใช้ในโค้ดของฉันเพื่อให้โปรแกรมแสดงผลตามที่โจทย์กำหนด โดยการสั่ง
print(*result[::-1]) เพื่อให้เรียงคำตอบจากมากไปน้อยและเว้นด้วย " "
```
