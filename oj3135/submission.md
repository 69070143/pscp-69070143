...
```text
เขียนโปรแกรมคำนวณว่าจะมีคนที่ได้พิจารณาของขวัญกี่คน
input:
รับค่า รับจำนวนคนในห้อง(n) จำนวนคนถัดไปที่ส่งต่อ(k) และหมายเลขของคนที่เป็นขโมย(thief)

output:
แสดงผล จำนวนคนที่ได้พิจารณาของขวัญ(count)

constraints:
วนโดยนั่งเป็นวงกลม เริ่มที่คนที่ 1
```
...
```text
Step 1: รับค่าข้อมูล (text) โดยเก็บเป็น List และแยกข้อมูลด้วย .split()
Step 2: กำหนดให้ n = int(text[0])
Step 3: กำหนดให้ k = int(text[1])
Step 4: กำหนดให้ thief = int(text[2])
Step 5: กำหนดให้ count = 0
Step 6: กำหนดให้ num = 1
Step 7: ใช้คำสั่ง while True: เพื่อเริ่มวนลูป
Step 8: นับจำนวนครั้งที่มีคนได้ตรวจของขวัญโดยให้ count += 1
Step 9: เช็คว่า num == thief หรือไม่
Step 10: ถ้าใช่ ให้ออกจากลูปด้วย break
Step 11: คำนวณตำแหน่งคนถัดไปโดยให้ num = num + k
Step 12: เช็คว่า num == 1 หรือไม่
Step 13: ถ้าใช่ ให้ออกจากลูปด้วย break
Step 14: พิมพ์ค่า count

ก่อนถามเพื่อนฉันไม่มั่นใจว่าโค้ดการคำนวณเลข step 11 ของฉันถูกต้องไหม
```
...
Step 9: เช็คว่า num == thief หรือไม่
Step 10: ถ้าใช่ ให้ออกจากลูปด้วย break
Step 11: คำนวณตำแหน่งคนถัดไปโดยให้ num = ((num + k - 1) % n) + 1
Step 12: เช็คว่า num == 1 หรือไม่
Step 13: ถ้าใช่ ให้ออกจากลูปด้วย break
Step 14: พิมพ์ค่า count

ฉันเปลี่ยนโค้ด step 11 จากเดิมที่ฉันใช้วิธีคิดผิด ก็แก้ไขให้ถูกต้อง
```
...
```text
ช่วยสอนขั้นตอนการคำนวณนับเลขวนเป็นวงกลม และช่วยอธิบายจนฉันเข้าใจ
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันนำแนวทางที่เพื่อนสอนมาแปลงเป็นโค้ด คือ num = ((num + k - 1) % n) + 1
```
