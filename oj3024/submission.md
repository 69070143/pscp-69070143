...
output:
แสดงผลคำว่า "Not surprising"
หรือ แสดงผลคำว่า "Surprising" ถ้าคะแนนสูงสุดและต่ำสุดห่างกันมากกว่า 2 คะแนน
```
...
```text
Step 1: รับค่าผลรวมทั้งหมดของคะแนนโหวด (total_score)
Step 2: รับค่าคะแนนโหวดสูงสูด (highest)
Step 3: กำหนดค่าคะแนนต่ำสุด (lowest) = total_score - (highest * 2)
Step 4: เช็คว่า highest - lowest > 2 หรือไม่
Step 5: ถ้าจริง ให้แสดงผลคำว่า "Surprising"
Step 6: ถ้าไม่จริง ให้แสดงผลคำว่า "Not surprising"

ก่อนถามเพื่อนมี testcase ที่ยังไม่ผ่านอยู่
```
...
```text
หลังจากขอคำปรึกษากับเพื่อน เพื่อนบอกฉันว่าอาจจะมีกรณีที่ค่าน้อยสุดอาจเป็นติดลบได้จากการคำนวณ
ฉันจึงเพิ่ม if เพื่อเช็คหลังจากคำนวณค่า lowest ว่าค่า lowest < 0 หรือไม่
ถ้าจริง จะกำหนดให้ lowest = 0 
(เพราะอาจจะมีกรณีที่ lowest มีค่าน้อยกว่า 0 เพราะค่าน้อยสุดที่โหวดได้คือ 0)
```
...
```text
เมื่อคำนวณแล้ว lowest จะมีค่าน้อยกว่า 0
```
...
```text
highest - lowest มีค่าน้อยกว่า 2
```
...
```text
ช่วยไกด์ให้ว่าโค้ดของเราอาจมีข้อผิดพลาดในกรณีที่คำนวณค่า lowest ออกมาแล้วมีค่าติดลบ
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันแก้ไขโค้ดของตัวเองโดยเพิ่มขั้นตอนเพื่อเช็คหลังจากคำนวณค่า lowest ว่าค่า lowest < 0 หรือไม่
ถ้าจริง จะกำหนดให้ lowest = 0 ให้คำตอบที่ออกมาไม่คลาดเคลื่อน เพราะโจทย์กำหนดว่ามีค่าได้แค่ 0-10 เท่านั้น
```
