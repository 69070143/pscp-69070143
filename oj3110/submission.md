...
```text
คํานวณค่าส่งพัสดุของ ต้นทาง-ปลายทาง ที่กำหนดไว้ 
input:
รับค่า ต้นทาง ปลายทาง
รับค่า น้ำหนักพัสดุ

output:
แสดงค่าส่ง (ทศนิยม 2 ตำแหน่ง)
ถ้าเส้นทางไม่อยู่ในตารางให้แสดงคำว่า "Error"
```
...
```text
Step 1: รับค่าสถานที่ต้นทางและปลายทาง (place) โดยเก็บเป็นList และแยกข้อมูลด้วย .split()
Step 2: กำหนดให้ start = สถานที่ต้นทาง (place[0])
Step 3: กำหนดให้ end = สถานที่ปลายทาง (place[1])
Step 4: รับค่า weight (น้ำหนัก)
Step 5: กำหนดให้ price = 0
Step 6: เช็คว่า start == "BKK" และ end == "CNX" หรือไม่
Step 7: ถ้าใช่ ให้คำนวณ price = (weight * 30) + 10
Step 8: ถ้าไม่ใช่ เช็คต่อว่า start == "CNX" และ end == "UBP" หรือไม่
Step 9: ถ้าใช่ ให้คำนวณ price = (weight * 40) + 15
Step 10: ถ้าไม่ใช่ เช็คต่อว่า start == "UBP" และ end == "BKK" หรือไม่
Step 11: ถ้าใช่ ให้คำนวณ price = (weight * 40) + 20
Step 12: ถ้าไม่ใช่ เช็คต่อว่า start == "BKK" และ end == "PKT" หรือไม่
Step 13: ถ้าใช่ ให้คำนวณ price = (weight * 50) + 25
Step 14: ถ้าไม่ใช่ เช็คต่อว่า start == "PKT" และ end == "CNX" หรือไม่
Step 15: ถ้าใช่ ให้คำนวณ price = (weight * 60) + 30
Step 16: ถ้าไม่ใช่ เช็คต่อว่า start == "UBP" และ end == "PKT" หรือไม่
Step 17: ถ้าใช่ ให้คำนวณ price = (weight * 70) + 40
Step 18: ถ้าไม่ตรงกับเงื่อนไขใดๆ เลย ให้แสดงผล "Error"
Step 19: เช็คว่า price > 0 หรือไม่
Step 20: ถ้าใช่ ให้แสดงผลค่า price (เป็นทศนิยม 2 ตำแหน่ง)
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก โดยใช้ if else ในการตรวจสอบเงื่อนไขว่าต้นทาง-ปลายทางคือที่ไหน จากนั้นแสดงผลค่าพัสดุ ถ้าไม่ตรงเงื่อนไขไหนเลยจะแสดงผล "Error"
```
...
```text
เคสที่ไม่มีข้อมูล ต้นทาง-ปลายทาง ที่ input มาอยู่ในตาราง
```
